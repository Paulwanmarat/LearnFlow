...
function detectSubject(text = "") {
  const keywords = [
    ...
    "node", "coding", "programming", "algorithm",
    "html", "css", "software", "backend", "frontend"
  ];
...
`;
    ...
    while (questions.length < questionAmount && attempts < 3) {
      ...
      let result;
      try {
        result = await callAI(basePrompt(needed), 1600);
      } catch {
        continue;
      }
      ...
      for (let q of parsed) {
        ...
        if (q.type === "mcq") {
          ...
          const invalidOptions = q.options.every(opt =>
            ["a", "b", "c", "d"].includes(opt.trim().toLowerCase())
          );
          if (invalidOptions) continue;

          if (!q.options.includes(q.answer)) {
            q.options.unshift(q.answer);
          }
          ...
          if (!q.explanation || typeof q.explanation !== "object") {
            q.explanation = { correct: "", incorrect: {} };
          }

          if (!q.explanation.correct)
            q.explanation.correct =
              "This answer correctly applies the concept.";
          ...
          q.options.forEach(opt => {
            if (opt !== q.answer && !q.explanation.incorrect[opt]) {
              q.explanation.incorrect[opt] =
                "This option does not correctly apply the concept.";
            }
          });
        }
...
exports.submitQuiz = async (req, res) => {
  ...
  if (activeSubmissions.has(userId)) {
    return res.status(429).json({ error: "Already submitting" });
  }
  ...
  try {
    ...
    const grading = await Promise.all(
      answers.map(async (item) => {
        ...
        /* ---------- MCQ ---------- */
        if (item.type === "mcq") {
          const userAns = normalizeAnswer(item.userAnswer);
          const correctAns = normalizeAnswer(item.correctAnswer);
          ...
          if (userNum !== null && correctNum !== null) {
            isCorrect = userNum === correctNum ? 1 : 0;
          } else {
            isCorrect = await gradeWithAI(
              item.question,
              item.correctAnswer,
              item.userAnswer
            );
          ...
          try {
            ...
            const modelOut = await executeCode(
              item.language || "javascript",
              item.correctAnswer
            );
    ...
    if (!user.lastQuizDate) {
      ...
      const diff = Math.floor(
        (today - new Date(user.lastQuizDate)) / 86400000
      );
...
Return explanation in this exact structure:
...
`;
  ...
  try {
    ...
    return "Incorrect answer. Review the concept and compare with the correct answer.";
  }
}
...
function unlockAchievements(user) {
  ...
  if (user.xp >= 100 && !has("Rookie"))
    user.achievements.push({ name: "Rookie" });

  if (user.xp >= 500 && !has("Pro Learner"))
    user.achievements.push({ name: "Pro Learner" });

  if (user.streak >= 7 && !has("7 Day Streak"))
    user.achievements.push({ name: "7 Day Streak" });

  if (user.level >= 10 && !has("Level 10 Master"))
...
`;
    ...
    while (questions.length < QUESTION_COUNT && attempts < 3) {
      ...
      let result;
      try {
        result = await callAI(prompt, 1600);
      } catch {
        continue;
      }
    ...
    if (questions.length < QUESTION_COUNT) {
      return res.status(500).json({
        error: "AI failed to generate weak questions"
      });
    }
